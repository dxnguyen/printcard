...
class RequestModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_REQUESTS';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_requests.request';
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_requests'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__requests');
				$max             = $db->loadResult();
