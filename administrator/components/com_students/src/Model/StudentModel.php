...
class StudentModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_students.student';
	...
	/**
	 * Method to duplicate an Student
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_students'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__students');
				$max             = $db->loadResult();
