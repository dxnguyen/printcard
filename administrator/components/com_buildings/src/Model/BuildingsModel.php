...
/**
 * Methods supporting a list of Buildings records.
 *
 * @since  1.0.0
 */
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.name LIKE ' . $search . '  OR  a.building_group LIKE ' . $search . ' )');
			}
		}
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->building_group))
			{
				...
				foreach ($values as $value)
				{
					...
					{
						$db = $this->getDbo();
						$query = "select id,name from #__building_groups where id = '$value' AND state=1";
						$db->setQuery($query);
