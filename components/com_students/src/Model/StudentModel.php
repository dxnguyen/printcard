 ...
 * @since  1.0.0
 */
class StudentModel extends ItemModel
{
	public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_students')) && (!$user->authorise('core.edit', 'com_students')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_STUDENTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_STUDENTS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->building_group) && $this->_item->building_group != '')
		{
			if (is_object($this->_item->building_group))
			{
				$this->_item->building_group = ArrayHelper::fromObject($this->_item->building_group);
			}

			$values = (is_array($this->_item->building_group)) ? $this->_item->building_group : explode(',',$this->_item->building_group);
			...
			$this->_item->building_group = !empty($textValue) ? implode(', ', $textValue) : $this->_item->building_group;
		}

		if (isset($this->_item->building) && $this->_item->building != '')
		{
			if (is_object($this->_item->building))
			{
				$this->_item->building = ArrayHelper::fromObject($this->_item->building);
			}

			$values = (is_array($this->_item->building)) ? $this->_item->building : explode(',',$this->_item->building);
			...
			$this->_item->building = !empty($textValue) ? implode(', ', $textValue) : $this->_item->building;
		}
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('student');
		}
