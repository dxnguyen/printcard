 ...
 * @since  1.0.0
 */
class RequestModel extends ItemModel
{
	public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_requests')) && (!$user->authorise('core.edit', 'com_requests')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_REQUESTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_REQUESTS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->requester_id))
		{
			$user = $userFactory->loadUserById($this->_item->requester_id);
			$this->_item->requester_id_name = $user->name;
		}

			if (isset($this->_item->requester_deparment))
			{
				$this->_item->requester_deparment = RequestsHelper::getGroupNamesById($this->_item->requester_deparment);
			}
		...
		if (isset($this->_item->type_id) && $this->_item->type_id != '')
		{
			...
			{
				$this->_item->type_id = ArrayHelper::fromObject($this->_item->type_id);
			}
		...
		if (isset($this->_item->technician_id))
		{
			$user = $userFactory->loadUserById($this->_item->technician_id);
			$this->_item->technician_id_name = $user->name;
		}

			if (isset($this->_item->tech_department))
			{
				$this->_item->tech_department = RequestsHelper::getGroupNamesById($this->_item->tech_department);
			}
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('request');
		}
